abstract class DashboardScreenText {
  static const String title = 'لوحة التحكم';
  static const String companyName = 'لايف بلاست للصناعات البلاستيكية';
  static const String systemSubtitle = 'نظام إدارة الحسابات والمخزون';

  // KPI Cards Titles
  static const String totalSales = 'إجمالي المبيعات';
  static const String totalPurchases = 'إجمالي المشتريات';
  static const String inventoryValue = 'قيمة المخزون';
  static const String itemsCount = 'عدد الأصناف';

  // Table Headers
  static const String inventorySummary = 'ملخص المخزون';
  static const String itemCode = 'كود الصنف';
  static const String itemName = 'اسم الصنف';
  static const String category = 'التصنيف';
  static const String quantity = 'الكمية';
  static const String minLimit = 'الحد الأدنى';
  static const String unit = 'الوحدة';
  static const String status = 'الحالة';
}